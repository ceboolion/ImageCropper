...
#if DEBUG
/// ViewModel zarządzający logiką przycinania obrazu
@Observable
final class ImageCropperViewModel {
    ...
    var cropRect: CGRect = .zero {
        didSet {
            print("📐 cropRect updated: \(cropRect)")
        }
    }
    var imageSize: CGSize = .zero {
        didSet {
            print("🖼️ imageSize updated: \(imageSize)")
        }
    }
    var containerSize: CGSize = .zero {
        didSet {
            print("📦 containerSize updated: \(containerSize)")
        }
    }
    ...
    init(image: PlatformImage) {
        self.image = image
        print("🎨 ImageCropperViewModel initialized with image size: \(image.size)")
    }
    
    // MARK: - Image Size Calculation
    
    func calculateImageSize(containerSize: CGSize) {
        print("📊 calculateImageSize called with containerSize: \(containerSize)")
        ...
        print("📊 effectiveSize (after padding): \(effectiveSize)")
        ...
        print("📊 imageAspect: \(imageAspect), containerAspect: \(containerAspect)")
        ...
        print("📊 displaySize calculated: \(displaySize)")
        ...
        if isFirstCalculation {
            ...
            cropRect = imageFrame
            print("✨ Initial cropRect set to image bounds: \(cropRect)")
            ...
            print("🔄 cropRect recalculated on orientation change")
        }
    }
    ...
    func startDragging() {
        ...
        isDragging = true
        print("👆 Started dragging from: \(initialCropRect)")
    }
    ...
    func resetToSquare() {
        ...
        print("⬜️ Reset to square: \(cropRect)")
    }
    ...
    func resetToFull() {
        ...
        cropRect = imageFrame
        print("🖼️ Reset to full: \(cropRect)")
    }
    
    // MARK: - Crop Execution
    
    func performCrop() throws -> CroppedImageResult {
        print("✂️ performCrop called")
        ...
        print("✂️ actualImageSize (pixels): \(actualImageSize)")
        print("✂️ image.size (points): \(image.size)")
        #if canImport(UIKit)
        ...
        print("✂️ imageFrame: \(imageFrame)")
        print("✂️ cropRect: \(cropRect)")
        
        // Znajdź przecięcie cropRect z obrazem
        let intersection = cropRect.intersection(imageFrame)
        
        print("✂️ intersection: \(intersection)")
        ...
        print("✂️ scaleX: \(scaleX), scaleY: \(scaleY)")
        ...
        print("✂️ imageCropRect (final): \(imageCropRect)")
